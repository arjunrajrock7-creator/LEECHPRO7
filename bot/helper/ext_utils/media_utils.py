...
class MediaUtils:
    ...
    async def process_custom_ffmpeg(path, cmd_list, listener):
        ...
        if await aiopath.isfile(path):
            ...
            for root, _, fs in await sync_to_async(os.walk, path):
                for f in fs:
        ...
        for f_path in files:
            ...
            for cmd_str in cmd_list:
                ...
                for p in parts:
                    if p.startswith("mltb."):
                        ...
                        if p_ext == "video" or p_ext == "audio":
                            ...
                            new_parts.append(os.path.join(os.path.dirname(curr_f_path), f"{os.path.basename(base_name)}.{p_ext}"))
                    elif p == "mltb":
                        new_parts.append(curr_f_path)
                    else:
                        new_parts.append(p)
                ...
                if success:
                    if should_del and await aiopath.exists(curr_f_path) and output_path and os.path.abspath(curr_f_path) != os.path.abspath(output_path):
                        await aioremove(curr_f_path)
                    if output_path and await aiopath.exists(output_path):
